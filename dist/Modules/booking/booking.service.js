...
const createBookingToDB = async (payload) => {
    ...
    const vehicleResult = await db_1.pool.query(`SELECT id, daily_rent_price, availability_status
     FROM vehicles 
    ...
    const conflict = await db_1.pool.query(`SELECT * FROM bookings 
     WHERE vehicle_id = $1
     AND NOT (rent_end_date < $2 OR rent_start_date > $3)`, [vehicle_id, rent_start_date, rent_end_date]);
    ...
    const newBooking = await db_1.pool.query(`INSERT INTO bookings (
      customer_id, 
      vehicle_id, 
      rent_start_date, 
      rent_end_date, 
...
const getBookingFromDB = async (payload) => {
    const { id, role } = payload;
    let result;
    if (role === "admin") {
        result = await db_1.pool.query("SELECT * FROM bookings");
    }
    else {
        result = await db_1.pool.query("SELECT * FROM bookings WHERE customer_id = $1", [
            id,
        ]);
    }
    return result;
};
const updateBookingInDB = async (bookingId, payload) => {
    ...
    if (role === "customer") {
        ...
        const cancelRes = await db_1.pool.query(`UPDATE bookings 
    ...
    if (role === "admin") {
        const updatedBooking = await db_1.pool.query(`UPDATE bookings 
       SET status = 'returned'
       WHERE id = $1
       RETURNING *`, [bookingId]);
        await db_1.pool.query(`UPDATE vehicles
